...
private:
    ...
    int height(Node<K, V> *node) const {
        ...
        if (node == NULL){
            ...
            counter = std::max(height(node->right),height(node->left));
            counter++;
        }
        return counter;
    }
   ...
   size_t leaf_count(Node<K, V> *node) const {
        ...
        counter += leaf_count(node->left);
        counter += leaf_count(node->right);
        return counter;
    }
    /**
    ...
    int diameter(Node<K, V> *node) const {
        ...
        int maxdiameter = std::max(diameter(node->left), diameter(node->right));
        return std::max(height(node->left) + height(node->right) + 2, maxdiameter);
    }
